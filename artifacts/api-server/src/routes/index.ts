import { Router, type IRouter } from "express";
import brandsRouter from "./brands";
import templatesRouter from "./templates";
import assetsRouter from "./assets";
import hashtagSetsRouter from "./hashtag-sets";
import creativesRouter from "./creatives";
import creativeVariantsRouter from "./creative-variants";
import stagesRouter from "./stages";
import directionRouter from "./direction";
import exploreRouter from "./explore";
import copyRouter from "./copy";
import brandRecordRouter from "./brand-record";
import teamRouter from "./team";
import calendarEntriesRouter from "./calendar-entries";
import socialAccountsRouter from "./social-accounts";
import uploadRouter from "./upload";
import generateRouter from "./generate";
import downloadRouter from "./download";
import socialAuthRouter from "./social-auth";
import videoRouter from "./video";
import costLogsRouter from "./cost-logs";
import postMetricsRouter from "./post-metrics";
import settingsRouter from "./settings";
import rewriteRouter from "./rewrite";
import conceptSuggestionsRouter from "./concept-suggestions";
import intentRouter from "./intent";
import brandAssistRouter from "./brand-assist";
import contentPlanRouter from "./content-plan";
import brandReadinessRouter from "./brand-readiness";
import scheduleProfileRouter from "./schedule-profile";
import smartScheduleRouter from "./smart-schedule";
import feedbackRouter from "./feedback";
import usersRouter from "./users";
import publishHealthRouter from "./publish-health";
import insightsRouter from "./insights";
import performanceConclusionsRouter from "./performance-conclusions";
import sequencesRouter from "./sequences";
import sequenceAudioRouter from "./sequence-audio";
import sequenceRenderRouter from "./sequence-render";
import storyboardRouter from "./storyboard";
import layersRouter from "./layers";
import layersDetectRouter from "./layers.detect";
import savedRunsRouter from "./saved-runs";
import shipRouter from "./ship";
import channelsRouter from "./channels";
import briefAssistRouter from "./brief-assist";
import smartBarRouter from "./smart-bar";
import signalsRouter from "./signals";
import tasteRouter from "./taste";
import designerPersonasRouter from "./designer-personas";
import sessionsRouter from "./sessions";

const router: IRouter = Router();

router.use(brandsRouter);
router.use(templatesRouter);
router.use(assetsRouter);
router.use(hashtagSetsRouter);
router.use(creativesRouter);
router.use(creativeVariantsRouter);
router.use(stagesRouter);
router.use(teamRouter);
router.use(directionRouter);
router.use(exploreRouter);
router.use(copyRouter);
router.use(brandRecordRouter);
router.use(calendarEntriesRouter);
router.use(socialAccountsRouter);
router.use(uploadRouter);
router.use(generateRouter);
router.use(downloadRouter);
router.use(socialAuthRouter);
router.use(videoRouter);
router.use(costLogsRouter);
router.use(postMetricsRouter);
router.use(settingsRouter);
router.use(rewriteRouter);
router.use(conceptSuggestionsRouter);
router.use(intentRouter);
router.use(brandAssistRouter);
router.use(contentPlanRouter);
router.use(brandReadinessRouter);
router.use(scheduleProfileRouter);
router.use(smartScheduleRouter);
router.use(feedbackRouter);
router.use(usersRouter);
router.use(publishHealthRouter);
router.use(insightsRouter);
router.use(performanceConclusionsRouter);
router.use(sequencesRouter);
router.use(sequenceAudioRouter);
router.use(sequenceRenderRouter);
router.use(storyboardRouter);
router.use(layersRouter);
router.use(layersDetectRouter);
router.use(savedRunsRouter);
router.use(shipRouter);
router.use(channelsRouter);
router.use(briefAssistRouter);
router.use(smartBarRouter);
router.use(signalsRouter);
router.use(tasteRouter);
router.use(designerPersonasRouter);
router.use(sessionsRouter);

router.all("/{*path}", (_req, res) => {
  res.status(404).json({ error: "Endpoint not found" });
});

export default router;
