...
/**
 * smart-bar invariants, under vitest so CI runs them. The cases live in
 * smart-bar.cases.ts, shared with the tsx runner; each becomes its own named
 * test so a failure names the invariant, not the file.
 */

const cases = collectSmartBarCases();
