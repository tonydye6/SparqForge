/**
 * The shared case list for the stage dependency engine.
 *
 * Spec: SparqMake Sandbox/20_SPEC_00_PRINCIPLES.md §1.1 to §1.5
 *
 * Two consumers run these identical cases, which is why they live here rather
 * than in either one:
 *
 *   stage-graph.test.ts    · vitest, so CI on Replit runs them
 *   stage-graph.verify.ts  · a plain tsx runner, because vitest cannot run on
 *                            the development Mac (the Linux-resolved lockfile
 *                            omits @rollup/rollup-darwin-arm64)
 *
 * Duplicating 63 assertions across two files would guarantee they drift, and
 * the whole point of this engine is that it is the one thing in the repo that
 * can be exhaustively checked. One list, two runners.
 */
...
export interface StageGraphCase {
  name: string;
  ok: boolean;
  /** Only populated on failure, to keep the passing path allocation-free. */
  detail?: unknown;
}

/**
 * Runs every case and returns the results. Pure and eager: the engine has no
 * IO, so evaluating all of them costs microseconds and lets both runners treat
 * the output as plain data.
 */
export function collectStageGraphCases(): StageGraphCase[] {
  const results: StageGraphCase[] = [];

  function check(name: string, condition: boolean, detail?: unknown) {
    results.push(condition ? { name, ok: true } : { name, ok: false, detail });
  }
